        ...
        if(source instanceof Player player) {
            if(Permission.hasPermission(player,"essentials.command.friends")) {
                if(Players.getPlayer(player).getFriends().isEmpty()) {
                    player.sendMessage(VEssentials.plugin.prefix.append(Component.text("§cYou don't have any friends yet!")));
                }else {
                   ...
                   String friends = builder.substring(0,builder.length() - 1);
                    player.sendMessage(VEssentials.plugin.prefix.append(Component.text( "§8List of you friends: §1" + friends + "§8!")));
                }
