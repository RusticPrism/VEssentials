        ...
        if(source instanceof Player player) {
            if(player.hasPermission("essentials.command.friends")) {
                if(Players.getPlayer(player).getFriends().isEmpty()) {
                    player.sendMessage(Component.text(VEssentials.plugin.prefix + "§cYou don't have any friends yet!"));
                }else {
                   ...
                   String friends = builder.substring(0,builder.length() - 1);
                    player.sendMessage(Component.text(VEssentials.plugin.prefix + "§8List of you friends: §1" + friends + "§8!"));
                }
