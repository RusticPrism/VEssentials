            ...
            if (player.hasPermission("essentials.command.friends")) {
                if (args.length == 1) {
                    if(VEssentials.plugin.server.getPlayer(args[0]).isPresent()) {
                        ...
                        player.sendMessage(Component.text(VEssentials.plugin.prefix + "§8Declined the Friend Request from §1" + VEssentials.plugin.server.getPlayer(args[0]).get().getUsername() + " §8!"));
                    }else player.sendMessage(Component.text(VEssentials.plugin.prefix + "§cCouldn't decline the Friend Request because this Player is Offline (Fixed soon)"));
                }else player.sendMessage(Component.text(VEssentials.plugin.prefix + "§cTo many Arguments!"));
