            ...
            if(Permission.hasPermission(player,"essentials.command.friends")) {
                if (args.length == 1) {
                    if (VEssentials.plugin.server.getPlayer(args[0]).isPresent()) {
                        ...
                        player.sendMessage(VEssentials.plugin.prefix.append(Component.text("§8Successfully accepted the friend request of §1" + VEssentials.plugin.server.getPlayer(args[0]).get().getUsername())));
                    } else player.sendMessage(VEssentials.plugin.prefix.append(Component.text("§cCouldn't accept the Friend Request because this Player is Offline (Fixed soon)")));
                }
