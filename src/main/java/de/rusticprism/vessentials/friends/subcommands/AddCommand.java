        ...
        if(source instanceof Player player) {
            if(Permission.hasPermission(player,"essentials.command.friends")) {
                if(VEssentials.PLUGIN.server.getPlayer(args[0]).isEmpty()) {
                    player.sendMessage(Component.text(Messages.prefix + "§cThis Player is not online!"));
                }
                Players.getPlayer(VEssentials.PLUGIN.server.getPlayer(args[0]).get()).sendRequest(player);
                player.sendMessage(Messages.prefix.append(Component.text( "§8Sent a friend request to §1" + VEssentials.PLUGIN.server.getPlayer(args[0]).get().getUsername()+ " §8!")));
